...
def run_sarima(train, test, target_col):
    """Pure SARIMA model (Baseline): Captures daily and weekly seasonality."""
    model = SARIMAX(train[target_col], 
                    order=(1, 1, 1), 
                    seasonal_order=(1, 1, 1, 7))
    results = model.fit(disp=False)
    return results.get_forecast(steps=len(test)).predicted_mean

def run_sarimax(train, test, target_col, exog_cols):
    ...
    model = SARIMAX(train[target_col], 
                    ...
                    seasonal_order=(1, 1, 1, 7))
    results = model.fit(disp=False)
    return results.get_forecast(steps=len(test), exog=test[exog_cols]).predicted_mean

def run_random_walk(train, test, target_col):
...
def main():
    ...
    for store_id in stores:
        ...
        test_store = test_full[test_full['Store ID'] == store_id]

        try:
            # 2. Execute all models
            p_rw = run_random_walk(train_store, test_store, target)
            p_sarima = run_sarima(train_store, test_store, target)
            p_sarimax = run_sarimax(train_store, test_store, target, exog_vars)

            # 3. Calculate metrics (MAE)
            mae_rw = mean_absolute_error(test_store[target], p_rw)
            mae_sarima = mean_absolute_error(test_store[target], p_sarima)
            mae_sarimax = mean_absolute_error(test_store[target], p_sarimax)
