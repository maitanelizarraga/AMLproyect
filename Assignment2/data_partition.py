...
def partition_by_store(df):
    ...
    for store_id in stores:
        ...
        test_list.append(store_df.iloc[train_end:])
...
def partition_by_region(df):
    ...
    for region in regions:
        ...
        test_list.append(region_df.iloc[train_end:])
...
def partition_by_category(df):
    ...
    for category in categories:
        ...
        test_list.append(category_df.iloc[train_end:])
...
def partition_by_product(df):
    ...
    for product in Products:
        ...
        test_list.append(product_df.iloc[train_end:])
...
def main():
    ...
    train_store, test_store = partition_by_store(df_store)
    df_region = pd.read_csv(input_path_region, parse_dates=['Date'], index_col='Date')
    train_region, test_region = partition_by_region(df_region)
    df_category = pd.read_csv(input_path_category, parse_dates=['Date'], index_col='Date')
    train_category, test_category = partition_by_category(df_category)
    df_product = pd.read_csv(input_path_product, parse_dates=['Date'], index_col='Date')
    train_product, test_product = partition_by_product(df_product)
    ...
    print(f"Train: {len(train_store)} | Test: {len(test_store)}")
    print("Final datasets saved: train_Store.csv, test_Store.csv")
    print("\n") 

    print("--- PARTITION SUMMARY REGIONS ---")
    print(f"Train: {len(train_region)} | Test: {len(test_region)}")
