...
def imputation(df):
    ...
    # verbose=2 imprimirá cada iteración y el tiempo que tarda
    imputer = IterativeImputer(
        estimator=ExtraTreesRegressor(n_estimators=10, random_state=42),
        max_iter=20,
        random_state=42,
        add_indicator=False,
        verbose=2,  # <--- Cambiado a 2 para ver detalle de iteraciones
        imputation_order='ascending' # Imputa primero las columnas con menos nulos
    )
    
    print("\n" + "="*50)
    print("INICIANDO PROCESO DE IMPUTACIÓN PASO A PASO")
    print("="*50)
    
    imputed_data = imputer.fit_transform(df_temp[all_cols_to_impute])
    df_temp[all_cols_to_impute] = imputed_data

    # 4. Revertir encoding (igual que antes)
    for col in cat_cols:
        le = encoders[col]
    ...
    print("\n" + "="*50)
    print(f"Imputación completada con éxito. Nulos restantes:{nulos_finales}")
    print("="*50)
    return df_temp
...
def main():
    ...
    try:
        ...
        print(f"Dataset cargado: {df.shape} con {df.isnull().sum().sum()} nulos.")
